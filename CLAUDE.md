...
Global multi-project scripts live at `~/code/github/claude-config/scripts/`:
- `dctl` — per-project Docker Compose helper (auto-detects stg env, group-aware: `dctl up`, `dctl status`, `dctl logs <svc>`, `dctl restart <svc>`, `dctl list`, `dctl dns-setup`)
- `otus` — (check `--help` when needed)
- `consolidate-project-dirs.sh` — canonical project-dir resolver
- `generate-active-projects.sh`, `generate-routing.sh` — regenerate this file's tables

Always check here first before assuming a script doesn't exist.

## Response Brevity (default mode)

Keep responses as short as possible while still answering. Default to terse — expand only when the user explicitly asks ("explain", "go deeper", "walk me through"). Rules:
- No preamble ("Great question", "Sure, I can help with that", "Let me…").
- No trailing summaries of work the user can see in the diff/tool output.
- No bullet lists when one sentence works. No headers for short replies.
- State the result/decision first. Skip the reasoning unless asked.
- For confirmations: one line. For status: one line.
- Code changes: say what changed in ≤1 sentence; don't restate the diff.
- If a question needs a yes/no, lead with yes/no.

Override only when: user asks to expand, task is genuinely complex and requires multi-step reasoning the user needs to see, or you're flagging a risk/blocker that must be understood before proceeding.
