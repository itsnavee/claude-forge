...
Git root → derive project name:
  ~/code/github/my-project/app/ → "my-project"
  ~/code/github/my-store/ → "my-store"
  ~/code/github/my-infra/ → "my-infra"
...
After all writes to the second-brain repo are complete, `cd` to `~/code/github/second-brain/` and invoke `/my-git-sync` to stage, commit, and push all changes. The commit message should reference the project that was synced (e.g., "brain-sync: update my-project summary").
