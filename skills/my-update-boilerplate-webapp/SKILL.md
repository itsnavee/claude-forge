...
Push improvements from the current project (project-a, project-b, etc.) back to the shared `boilerplate-webapp` repo. Uses a helper script for mechanical copying and AI intelligence for complex genericization.
...
3. **Choose the sync mode:**
   ...
   - **Mode B — Sed genericization**: The file has ONLY simple brand-string differences (e.g., `project-theme` → `app-theme`, `MyApp` → `MyApp`). The replacement list in the config handles it.
     ```bash
...
**Source projects**: project-a, project-b (configured in `boilerplate-sync-config.json`).
